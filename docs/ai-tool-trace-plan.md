...
> 目标：只补可观测性，不改现有预约主链路。
>
> **实施状态（2026-08-19）：已落地。** 开关为请求头 `X-AI-Debug: 1`（或 `true`）；未开时响应不含 `debug`。契约见 [api-contract.md](./api-contract.md) §9.3，验收见 [ai-agent-test-case.md](./ai-agent-test-case.md) §5。
...
未采用请求体/query，避免污染现有 `{ "message" }` 契约。小程序默认不带头。
